...
export default class ProdDesc extends Component {
  ...
  async componentDidMount() {
    let results = await Axios(
      `https://webstorebackend.azurewebsites.net/api/productapi/${this.props.match.params.id}`
    );
        ...
        <Col sm={6}>
          <ResponsiveEmbed aspectRatio="4by3">
            <figure className="progressive">
              <Image
                data-progressive={this.state.ImgLoc}
                src={this.state.ImgLocMin}
                className="mr-3 progressive__img progressive--not-loaded"
                alt=""
                rounded
              />
            </figure>
