...
export default class UpdateQtyBtn extends Component {
  ...
  update = async (e) => {
    ...
    if (document.getElementById("NumInputstyle").value > 0) {
      ...
      let result = await Axios.post(
        "http://localhost:51129/api/cart/UpdateCart",
        Mydata
      ).then((result) => {
        console.log(result);
  ...
  render() {
    ...
    if (UpdateLinkClicked) {
      return (
        <div>
          <NumericInput
            ...
            value={0}
          />
          <a className="UpdateQty" href="" onClick={this.update}>
            Update
          </a>
          <a className="UpdateQty" href="" onClick={this.cancel}>
            Cancel
          </a>
          <Snackbar
            ...
            message={
              <span id="message-id">Please Select a value greater than 0</span>
            }
          />
        </div>
      );
      ...
      return (
        <div>
          <a className="UpdateQty" href="" onClick={this.handleClick}>
            Change Quantity
          </a>
          <Snackbar
