...
export default class DefBillAddr extends React.Component {
  state = { StreetAddress: "", city: "", State: "", ZipCode: "", country: "", UseDefBillAddr: false};
   ...
   GetAccountInfo = async () => { 
    ...
    //Makes API call to get Billing address for account user 
     results = await axios.get("http://localhost:51129/api/login/GetDefBillAddr",
     {
      ...
      else { await this.setState({UseDefBillAddr: false})}
      ...
      <Form>
        ...
        <FormControlLabel
            control={
              ...
              onChange={(event) => this.handleChange({UseDefBillAddr: event.target.checked })}  
              value="primary"
